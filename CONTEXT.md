...
**InboundEmail status**:
The processing lifecycle of an `InboundEmail` record: Pending → Receiving → Received → Processing → Processed, or Failed. One row is created per webhook and updated in place as it advances — never one row per transition. Tenant-policy rejections (unrecognized provider, disallowed tenant) are never recorded at all; the row only starts existing once tenant policy has already passed. A provider handler's signature verification failure *does* land on that row as `Failed`, with `error` set to `"Verification failed"` — the row exists first, verification runs against it, and failure is recorded rather than the request being dropped with no trace. A failure *after* verification (e.g. malformed MIME during parsing, or the app's job setting the status directly) lands on that same row as `Failed`, with `error` recording why.
...
A nullable reference to the consuming app's own tenant record, resolved against a single model class the app declares via config (`tenant_model`). The package never populates this itself — the host app sets it after the row exists. Distinct from `organization_alias`, which the package populates itself from one of two discovery strategies: the raw `{orgAlias}` route segment when multi-tenant routing (`organization_in_route`) is enabled, or — when route routing is absent and email-based tenancy (`email_based_tenancy_enabled`) is enabled — a tenant identifier parsed from the recipient address by the configured `tenant_resolver` (plus-addressing by default). Both strategies can be enabled at once; the route-derived value always wins on disagreement.
